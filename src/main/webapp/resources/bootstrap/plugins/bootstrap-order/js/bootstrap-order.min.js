	...
	var f = {
		ui : {
			createDialog : function() {
				var g = '<div class="panel panel-primary panel-order">'
						+ '   <div class="panel-heading">'
						+ "   </div>"
						+ '   <div class="panel-body">'
						+ '       <ul class="list-group order-list-title" order-type="list-title">'
						+ '           <li class="list-group-item">'
						+ '               <span order-type="name">OrginalNum - Name</span>'
						+ '               <span class="pull-right" order-type="realNum">RealNum</span>'
						+ "           </li>"
						+ "       </ul>"
						+ '       <ul class="list-group order-list" order-type="list"></ul>'
						+ "   </div>" + "</div>";
				return d(g)
			},
			createRow : function() {
				var g = '<li class="list-group-item" title="双击移除记录">'
						+ '   <div class="input-group">'
						+ '       <span class="form-control col-th-6 order-name" order-type="name"></span>'
						+ '       <span class="form-control form-split col-th-3"><span></span></span>'
						+ '       <span class="form-control col-th-3 order-num">'
						+ '           <input type="text" class="form-control order-num" value="1" title="1" order-type="num" />'
						+ "       </span>" + "   </div>" + "</li>";
				return d(g)
			},
			createTip : function() {
		...
		menu : {
			collapse : function(k, j, i) {
				var l = !!k.data("order-menu-expanse"), h = d(
						"[order-type='list']", k), g = d(
						"[order-type='list-title']", k);
				if (!l && !k.is(":animated")) {
					k.animate({
						"width" : 140
					}, 500, function() {
			...
			expanse : function(l, j, i) {
				var k = !l.data("order-menu-expanse"), h = d(
						"[order-type='list']", l), g = d(
						"[order-type='list-title']", l);
				if (!k && !l.is(":animated")) {
					l.animate({
						"width" : 320
					}, 500, function() {
			...
			showOperation : function() {
				var h = d(document), g = h.data("order-menu-operation")
						|| (function() {
							var i = f.ui
									.createTip()
									.html(
											'<span class="alert alert-info alert-dismissable">'
													...
													+ "   <p>操作说明:</p> "
													+ "   <p>1、可以点击方框通过拖拽方式进行排序操作</p> "
													+ "   <p>2、也可以在输入框中输入序号后按回车键或是移开焦点自动排序</p> "
													+ "   <p>   如果是向后排序则会插入到相比较小(或相同)的最近排序框之后</p> "
													+ "   <p>   如果是向前排序则会插入到相比较大(或相同)的最近排序框之前</p> "
													+ "</span>").appendTo(
											"body");
							i.find(".close").off("click").on("click",
									function() {
										i.fadeOut(300)
									});
		...
		bindEvt : function(j) {
			var g = d("[order-type='menu']", j), k = d(
					"[order-type='menu-expanse']", g), o = d(
					"[order-type='menu-collapse']", g), p = d(
					"[order-type='menu-operation']", g), r = d(
					"[order-type='list']", j), s = d(".list-group-item", r), h = d(
					"[order-type='num']", r), l = (function() {
				var t = j.data("order-methods") || {};
				return {
					sortByInputNumber : d.isFunction(t.sortByInputNumber) ? t.sortByInputNumber
							: function() {
							},
					removeItem : d.isFunction(t.removeItem) ? t.removeItem
							: function() {
							}
				}
			}());
			...
			var n = d(document), m = d("body"), i = {
				onMove : (function() {
					return d.isFunction(j.data("order-move-events-onMove")) ? j
							.data("order-move-events-onMove") : function() {
					}
				}()),
				onUp : (function() {
					return d.isFunction(j.data("order-move-events-onUp")) ? j
							.data("order-move-events-onUp") : function() {
					}
				}())
			}, q = {
				...
				mouseUp : function(u) {
					...
					m.off("selectstart", q.docSelected).removeClass(
							"select-none");
					i.onUp(t)
				},
		...
		itemHeight : (function() {
			var k = f.ui.createDialog().css("left", -9999).appendTo("body")
					.show(), h = d("[order-type='list']", k), g = f.ui
					.createRow().appendTo(h), i = f.ui.createRow().appendTo(h), j = +(g
					.css("margin-top") || "0px").replace(/px/, ""), l = +(g
					.css("margin-bottom") || "0px").replace(/px/, ""), m = g
					.outerHeight()
					+ j + l;
			k.remove();
			return m
				...
				.extend(
						...
						i,
						{
							prototype : {
								setting : {
									prop : {
										title : "Bootstrap Sort",
										name : "OrginalNum - Name",
										realNum : "RealNum"
									}
								},
								setProp : function(j) {
									d.extend(true, this.setting.prop, j);
									h.$dialog.find("[order-type='title']")
											.html(this.setting.prop.title);
									h.$dialog.find("[order-type='name']").html(
											this.setting.prop.name);
									h.$dialog.find("[order-type='realNum']")
											.html(this.setting.prop.realNum);
									return this
								},
								show : function() {
									var j = h.$dialog;
									j.show().css({
										"left" : "-" + j.outerWidth() + "px"
									}).animate({
										"left" : 0
									}, 500);
									return this
								...
								showMsg : function(k) {
									var j = f.ui.createTip()
											.appendTo(d("body"));
									j.append("<span>" + k + "</span>").fadeIn(
											300, function() {
												setTimeout(function() {
								...
								addItem : function(t) {
									if (this.containItem(t)) {
										return this
												.showMsg("Record already exists")
									}
									var k = h.$dialog, o = h.$list, p = f.ui
											.createRow(), j = h.itemArray, u = this, s = 0, m = null, r = {
										onMove : function(H, C, y) {
											var D = C.top
													+ (y === "down" ? 0
															: b.itemHeight / 2), I = C.top
													+ b.itemHeight
													- (y === "up" ? 0
															: b.itemHeight / 2), E, B, z, G = H
													.data("order-itemObject"), F = G.realNum, x = H
													.attr("orderId"), v, w, A = false;
											if (m !== null && s !== 0
													&& m === y) {
												if (y === "down"
														&& s > D - b.itemHeight
									...
									d("[order-type='name']", p).html(
											t.num + " - " + t.name).attr(
											"title", t.num + " - " + t.name);
									d("[order-type='num']", p).val(t.num).attr(
											"title", t.num);
									p.attr("orderId", "order_" + b.index++);
									o.append(p);
									k
											.data("order-move-events-onMove",
													r.onMove);
									k.data("order-move-events-onUp", r.onUp);
									k.data("order-methods", l);
									f.bindEvt(h.$dialog);
									q = {
										index : h.itemArray.length,
										$row : p,
										item : t,
										realNum : t.num
									};
								...
								recovery : function() {
									...
									for (k = 0, j = h.itemArray.length; k < j; k++) {
										...
										h.itemArray[k].$row.find(
												"[order-type='num']").val(
												h.itemArray[k].item.num)
									}
								...
								doSort : function(m, r) {
									...
									for (q = 0, k = h.itemArray.length; q < k; q++) {
										if (r
												&& r.attr("orderId") === h.itemArray[q].$row
														.attr("orderId")) {
											continue
										}
										h.itemArray[q].$row.stop().animate({
											top : b.itemHeight * q
										}, o ? 100 : 300).find(
												"[order-type='num']").val(
												h.itemArray[q].realNum)
									}
									return this
								},
								getData : function() {
