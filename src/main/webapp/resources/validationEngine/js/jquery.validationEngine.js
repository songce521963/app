...
(function($) {
	...
	var methods = {
		...
		attach : function(userOptions) {
			...
			options.validateAttribute = (form
			...
			form
					.off(
							...
							"["
									...
									+ "*=validate]:not([type=checkbox]):not([type=radio]):not(.datepicker)",
							methods._onFieldEvent);
			form.off("click", "[" + options.validateAttribute
					+ "*=validate][type=checkbox],["
					+ options.validateAttribute + "*=validate][type=radio]",
					methods._onFieldEvent);
			form.off(options.validationEventTrigger, "["
					+ options.validateAttribute
					+ "*=validate][class*=datepicker]", methods._onFieldEvent);
		 ...
		 * accordingly. Note: There is no ajax form validation with this method,
		 * only field ajax validation are evaluated
		 * 
		...
		validate : function(userOptions) {
			...
			if (element.is("form")
				...
				if (element.hasClass('validating')) {
					...
					// Should abort old validation and start new one. I don't
				...
				else if (options.onFieldFailure
		...
		updatePromptsPosition : function(event) {
			...
			if (!options)
				options = methods._saveOptions(form, options);
			form.find('[' + options.validateAttribute + '*=validate]').not(
					":disabled").each(
					function() {
						var field = $(this);
						...
						if (prompt)
							methods._updatePrompt(field, $(prompt), promptText,
									undefined, false, options, noAnimation);
					});
			return this;
		},
		/**
		...
		hide : function() {
			...
			if (!options)
				options = methods._saveOptions(form, options);
			var fadeDuration = (options && options.fadeDuration) ? options.fadeDuration
					: 0.3;
			var closingtag;

			if (form.is("form") || form.hasClass("validationEngineContainer")) {
				closingtag = "parentForm"
						+ methods._getClassName($(form).attr("id"));
			} else {
				closingtag = methods._getClassName($(form).attr("id"))
						+ "formError";
			}
					...
					.each(
							function() {
								...
								if ($.inArray(field.attr('name'), names) < 0) {
									...
									if (errorFound && first_err == null)
										if (field.is(":hidden")
											...
											first_err = field = form.find("#"
													+ options.usePrefix
													+ methods._jqSelector(field
															.attr('id'))
													+ options.useSuffix);
										else {

											// Check if we need to adjust what
											// element to show the prompt on
									...
									// if option set, stop checking validation
									// rules after one error is found
									if (options.showOneMessage == true
											&& errorFound) {
										return false;
									}
								}
		...
		_validateFormWithAjax : function(form, options) {
			...
			var type = (options.ajaxFormValidationMethod) ? options.ajaxFormValidationMethod
					: "GET";
					...
					.ajax({
						...
						success : function(json) {
							...
							if ((dataType == "json") && (json !== true)) {
								...
								options.onAjaxFormComplete(true, form, json,
										options);
		 ...
		 * @param {Map}
		 *            user options
		 * @return false if field is valid (It is inversed for *fields*, it
		 *         return false on validate and true on errors.)
		 */
		_validateField : function(field, options, skipAjaxValidation) {
			...
			if (!options.validateNonVisibleFields
					&& (field.is(":hidden") && !options.prettySelect || field
							.parent().is(":hidden")))
				return false;
			...
			for (var i = 0; i < rules.length; i++) {
				rules[i] = rules[i].toString().replace(" ", "");// .toString to
																// worked on IE8
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				if (limitErrors && field_errors >= options.maxErrorsPerField) {
					// If we haven't hit a required yet, check to see if there
					// is one in the validation rules for this
					// field and that it's index is greater or equal to our
					// current index
					if (!required) {
				...
				case "required":
					required = true;
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._required);
					break;
				case "custom":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._custom);
					break;
				case "groupRequired":
					...
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._groupRequired);
					if (errorMsg)
						required = true;
				...
				case "minSize":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._minSize);
					break;
				case "maxSize":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._maxSize);
					break;
				case "min":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._min);
					break;
				case "max":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._max);
					break;
				case "past":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._past);
					break;
				case "future":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._future);
					break;
				case "dateRange":
					var classGroup = "[" + options.validateAttribute + "*="
							+ rules[i + 1] + "]";
					...
					if (options.firstOfGroup[0].value
						...
						errorMsg = methods
								._getErrorMessage(form, field, rules[i], rules,
				...
				case "dateTimeRange":
					...
					if (options.firstOfGroup[0].value
							|| options.secondOfGroup[0].value) {
						errorMsg = methods._getErrorMessage(form, field,
								rules[i], rules, i, options,
				...
				case "maxCheckbox":
					...
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._maxCheckbox);
					break;
				case "minCheckbox":
					field = $(form.find("input[name='" + fieldName + "']"));
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._minCheckbox);
					break;
				case "equals":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._equals);
					break;
				case "funcCall":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._funcCall);
					break;
				case "creditCard":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._creditCard);
					break;
				case "condRequired":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._condRequired);
				...
				case "funcCallRequired":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._funcCallRequired);
					if (errorMsg !== undefined) {
						required = true;
					}
					break;
				...
				// If we were passed back an message object, check what the
				// status was to determine what to do
				if (typeof errorMsg == "object") {
					switch (errorMsg.status) {
					case "_break":
						end_validation = true;
						break;
					// If we have an error message, set errorMsg to the error
					// message
					case "_error":
						errorMsg = errorMsg.message;
						break;
					// If we want to throw an error, but not show a prompt,
				...
				// funcCallRequired, first in rules, and has error, skip
				// anything else
				if (i == 0 && str.indexOf('funcCallRequired') == 0
						&& errorMsg !== undefined) {
					if (promptText != '') {
						promptText += "<br/>";
					}
				...
				// If it has been specified that validation should end now,
				// break
				if (end_validation) {
					break;
				}

				// If we have a string, that means that we have an error, so add
			...
			// validated
			// the 3rd condition is added so that even empty password fields
			// should be equal
			// otherwise if one is filled and another left empty, the "equal"
			...
			if (options.isError && options.showPrompts) {
				methods._showPrompt(field, promptText, promptType, false,
						options);
			} else {
				if (!isAjaxValidator)
		...
		_getErrorMessage : function(form, field, rule, rules, i, options,
			...
			// If we are using the custon validation type, build the index for
			// the rule.
			// Otherwise if we are doing a function call, make the call and
			// return the object
			// that is passed back.
			...
			if (rule === "custom" || rule === "funcCall"
				...
				// Delete the rule from the rules array so that it doesn't try
				// to call the
				// same rule over again
				delete (rules[rule_index]);
			}
			// Change the rule to the composite rule, if it was different from
			// the original
			var alteredRule = rule;

			var element_classes = (field.attr("data-validation-engine")) ? field
					.attr("data-validation-engine")
					: field.attr("class");
			var element_classes_array = element_classes.split(" ");

			// Call the original validation method. If we are dealing with dates
			// or checkboxes, also pass the form
			var errorMsg;
			if (rule == "future" || rule == "past" || rule == "maxCheckbox"
					|| rule == "minCheckbox") {
				errorMsg = originalValidationMethod(form, field, rules, i,
						options);
			} else {
		...
		_getCustomErrorMessage : function(field, classes, rule, options) {
			...
			var validityProp = /^custom\[.*\]$/.test(rule) ? methods._validityProp["custom"]
					: methods._validityProp[rule];
			// If there is a validityProp for this rule, check to see if the
			// field has an attribute for it
			if (validityProp != undefined) {
				custom_message = field
						.attr("data-errormessage-" + validityProp);
		 ...
		 *            user options
		 * @param {bool}
		 *            condRequired flag when method is used for internal purpose
		...
		_required : function(field, rules, i, options, condRequired) {
			...
			default:
				...
				var dv_placeholder = $.trim(field
						.attr("data-validation-placeholder"));
		...
		_groupRequired : function(field, rules, i, options) {
			...
			field.closest("form, .validationEngineContainer").find(classGroup)
					.each(function() {
		...
		/**
		 * Checks that it is a valid credit card number according to the Luhn
		 * checksum algorithm.
		 * 
		 * @param {jqObject}
		 *            field
		...
		_creditCard : function(field, rules, i, options) {
			// spaces and dashes may be valid characters, but must be stripped
		...
		_ajax : function(field, rules, i, options) {
			...
			if (extraDataDynamic) {
				...
				for (var i = 0; i < domIds.length; i++) {
					var id = domIds[i];
					if ($(id).length) {
						var inputValue = field.closest(
								"form, .validationEngineContainer").find(id)
								.val();
			...
			if (!options.isError
					&& !methods
							._checkAjaxFieldStatus(field.attr("id"), options)) {
				$
						.ajax({
							...
							success : function(json) {
								...
								if (errorField.length == 1) {
									...
									if (!status) {
										...
										// If a submit form triggered this, we
		...
		_buildPrompt : function(field, promptText, type, ajaxed, options) {
			...
			prompt.addClass("parentForm"
					+ methods._getClassName(field.closest(
							'form, .validationEngineContainer').attr("id")));
						...
						var prettyOverrideClass = $(
								'#'
										+ field
												.attr('id')
												.substr(
														options.usePrefix.length)
												.substring(
														options.useSuffix.length))
			...
			if ($('body').hasClass('rtl')) {
				prompt.css(
						{
							'position' : positionType === 'inline' ? 'relative'
									: 'absolute',
...
//					prompt.animate(css);
		...
		_getPrompt : function(field) {
			var formId = $(field).closest('form, .validationEngineContainer')
					.attr('id');
		...
		isRTL : function(field) {
			...
			var $body = $('body');
			var rtl = (field && field.hasClass('rtl'))
		...
		_condRequired : function(field, rules, i, options) {
			...
			for (idx = (i + 1); idx < rules.length; idx++) {
				...
				/*
				 * Use _required for determining wether dependingField has a
				 * value. There is logic there for handling all field types, and
				 * default value; so we won't replicate that here Indicate this
				 * special use by setting the last parameter to true so we only
				 * validate the dependingField on chackboxes and radio buttons
				 * (#462)
				 */
				if (dependingField.length
						&& methods._required(dependingField, [ "required" ], 0,
								options, true) == undefined) {
					/*
					 * We now know any of the depending fields has a value, so
					 * we can validate this field as per normal required code
					 */
	...
	$.fn.validationEngine = function(method) {
		...
		if (typeof (method) == 'string' && method.charAt(0) != '_'
			...
			if (method != "showPrompt" && method != "hide"
					&& method != "hideAll")
	...
	$.validationEngine = {
		...
		defaults : {
			...
			// in the form? (Useful in cases of tabbed containers, e.g.
			// jQuery-UI tabs)
			validateNonVisibleFields : false,
			// ignore the validation for fields with this specific class (Useful
			// in cases of tabbed containers AND hidden fields we don't want to
			// validate)
			...
			// Used when you have a form fields too close and the errors
			// messages are on top of other disturbing viewing messages
			doNotShowAllErrosOnSubmit : false,
			// Object where you store custom messages to override the default
			// error messages
			custom_error_messages : {},
			// true if you want to validate the input fields on blur event
			binded : true,
			// set to true if you want to validate the input fields on blur only
			// if the field it's not empty
			notEmpty : false,
			// set to true, when the prompt arrow needs to be displayed
			showArrow : true,
			// set to false, determines if the prompt arrow should be displayed
			// when validating
	...
	$(function() {
		$.validationEngine.defaults.promptPosition = methods.isRTL() ? 'topLeft'
				: "topRight"
	});
})(jQuery);
